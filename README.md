...
For existing companies, `GenericDiscoveryOrchestrator` sits before organization synthesis. It consumes neutral `BusinessSystemConnector` discoveries, turns observed systems into evidence/facts/capabilities, identifies unresolved universal business questions (revenue/customer flow, financial truth, delivery, authority boundaries and existing ownership), and returns prioritized owner questions. Until critical/high unknowns are resolved, discovery is explicitly incomplete; the orchestrator never creates departments, Work or authority.
