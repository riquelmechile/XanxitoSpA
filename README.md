...
For existing companies, `GenericDiscoveryOrchestrator` sits before organization synthesis. It consumes neutral `BusinessSystemConnector.describe()` results, turns observed systems into evidence/facts/capabilities, and returns prioritized owner questions plus readiness by scope (governance, commercial, finance, operations, organization). System existence can lower a question priority and provide candidate evidence, but it never proves financial truth, ownership or authority. Unknowns carry explicit resolution requirements (`evidence`, `operator-confirmation`, `owner-confirmation`, `constitutional-mandate`), and organization synthesis remains blocked until the relevant organization/governance scopes are sufficient.

Authority-bearing owner answers are not represented by `xspa.write` or by a mutable `owner-confirmed` flag. XanxitoSpA now has a generic, AP2-inspired verifiable mandate boundary: out-of-band server-configured Company trust anchors identify founder/owner/board roots, immutable `authority.mandate.1` envelopes bind company/issuer/scope/claims/time to an Ed25519 signature, and deterministic verification derives active state across expiry, supersession and revocation. Owner roots can issue signed delegation mandates that introduce scoped delegate public keys; revoking the delegation removes that derived authority without rewriting history. Only a verified active mandate can resolve `owner-confirmation` or `constitutional-mandate` unknowns, and the mandate must carry the corresponding scope. The system stores signed public evidence only; private signing keys are never accepted, persisted or returned. The root keyring is append-only in practice: every mandate apply persists a keyring head (count/hash plus public-key fingerprints), and verification fails closed with `AUTHORITY_KEYRING_INCOMPLETE` if a historical root key required by persisted history disappears or changes.

Production root enrollment remains outside ordinary MCP writes, but the bootstrap ceremony is now explicit and verifiable. `xspa_authority_root_enrollment_prepare` creates a short-lived Company-bound Ed25519 proof-of-possession challenge for a proposed Founder/Owner/Board public key, `xspa_authority_root_enrollment_verify` verifies the signature and returns a public-only enrollment bundle, and `xspa_authority_root_enrollment_status` exposes sanitized issued/consumed ceremony state without public keys or signatures. The ceremony persists bounded one-time challenge state: `prepare` refuses while another unexpired challenge is active, and the first `verify` attempt consumes the issued challenge whether the proof succeeds or fails. Status distinguishes `issued`, `expired` and `consumed` and exposes only a bounded recent history. Neither tool can persist or activate a trust anchor, both refuse when a root is already configured or mandate history exists, and successful verification returns `requiresOutOfBandProvisioning=true`. Final trusted public keys are still supplied through `XSPA_AUTHORITY_TRUST_ANCHORS_JSON`; therefore the principal able to modify deployment configuration is the effective root-provisioning authority. The signed enrollment bundle is proof-of-possession evidence, not active authority. If no root is configured, mandate verification reports `AUTHORITY_TRUST_NOT_CONFIGURED` and mandate application fails closed.
...
Production trust root    proof-of-possession ceremony ready; no real root enrolled yet; trustConfigured=false fails closed
Local test suite         118 PASS · PostgreSQL 18 smoke covers heartbeat/source cursor fencing, asset CAS + observed-signal replay ledger
MCP                      ChatGPT Streamable HTTP + OAuth + discovery/wake/authority/root-enrollment tools
