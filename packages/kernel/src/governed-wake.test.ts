...
describe("GovernedWakeEngine", () => {
  it("uses opportunity-cost plus action-window urgency and emits a no-authority Work proposal above threshold", () => {
    ...
    const result = engine.evaluate({ companyId, constitution: constitution(subscription()), events: [event("33333333-3333-4333-8333-333333333333", 0.9, 60, 30)], priorState: [], now });
  ...
  it("accumulates below threshold and replay does not increase the score", () => {
    const sub = subscription({ threshold: 0.3 });
    const engine = new GovernedWakeEngine();
    ...
    expect(first.state[0]?.score).toBeCloseTo(0.21, 5);
    ...
    expect(replay.state[0]?.score).toBeCloseTo(0.21, 5);
  ...
  it("keeps only a bounded defensive replay cache in accumulator state", () => {
    ...
    expect(first.state[0]?.processedEventKeys).toHaveLength(256);
