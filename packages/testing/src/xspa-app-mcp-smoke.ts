...
export async function verifyXspaAppMcp(): Promise<void> {
  ...
  try {
    ...
    const discoveryOrchestrate = await transport.callTool("xspa_company_discovery_orchestrate", { systems: [] }, metadata);
    assert(discoveryOrchestrate.ok && JSON.stringify(discoveryOrchestrate.content).includes("questions"), "company discovery orchestrate failed through app MCP");
