...
describe("verified authority mandates", () => {
  it("verifies a founder mandate and resolves owner-confirmation without trusting xspa.write", () => {
    ...
    const mandate = signMandate(unsigned());
    ...
    const next = applyVerifiedMandateToDiscovery(prior(), mandate, verification, new Date("2026-08-25T01:00:00.000Z"));
  ...
  it("supports explicit signed delegation and removes it when the delegation mandate is revoked", () => {
    ...
    const delegatedUnsigned = unsigned({ id: "99999999-9999-4999-8999-999999999999", issuerPrincipalId: "principal:cfo", scopes: ["company.discovery.confirm"] });
    ...
    expect(() => applyVerifiedMandateToDiscovery(prior(), delegated, verified, new Date("2026-08-25T01:00:00.000Z"))).not.toThrow();
  ...
  it("requires constitutional scope and trusted owner-class role for constitutional unknowns", () => {
    const { anchor, signMandate } = fixture();
    const mandate = signMandate(unsigned({ scopes: ["company.constitution"], claims: [{ type: "discovery-resolution", unknownId: "unknown:authority-boundaries", assertion: "Irreversible and financial actions require explicit approval." }] }));
    ...
    expect(verification.valid).toBe(true);
    expect(() => applyVerifiedMandateToDiscovery(prior("constitutional-mandate"), mandate, verification, new Date("2026-08-25T01:00:00.000Z"))).not.toThrow();
  });
});
